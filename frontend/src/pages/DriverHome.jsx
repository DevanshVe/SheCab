...
const DriverHome = () => {
    ...
    useEffect(() => {
        ...
        navigator.geolocation.getCurrentPosition(
            (pos) => {
                ...
                driverService.updateLocation(latitude, longitude).catch(console.error);
            },
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            if (isOnline) {
                navigator.geolocation.getCurrentPosition(
                    (pos) => {
                        driverService.updateLocation(pos.coords.latitude, pos.coords.longitude).catch(console.error);
                    },
                    err => console.error(err)
                );
            }
    ...
    const toggleOnline = async () => {
        try {
            const updatedUser = await driverService.toggleAvailability();
    ...
    const handleAccept = async (rideId) => {
        try {
            const ride = await bookingService.acceptRide(rideId);
            setActiveRide(ride);
            alert('Ride Accepted!');
            fetchRides(); // refresh
        } catch (error) {
        ...
        <div className="h-screen flex flex-col bg-gray-50 font-sans">
            <header className="bg-white shadow-sm p-5 flex justify-between items-center z-10">
                <div className="flex items-center gap-4">
                    <h1 className="text-2xl font-bold tracking-tight text-gray-900">HerWayCabs Driver</h1>
                    <button
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button onClick={logout} className="text-red-600 font-semibold text-sm hover:underline">Logout</button>
                </div>
            </header>

            <div className="flex-1 p-8 max-w-6xl mx-auto w-full overflow-auto">
                {activeRide ? (
                    <div className="bg-white p-8 rounded-2xl shadow-xl border border-blue-100 max-w-3xl mx-auto">
                        <div className="flex justify-between items-center mb-8">
                            <h2 className="text-3xl font-bold text-gray-900">Current Trip</h2>
                            <span className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold text-sm tracking-wide">
                                #{activeRide.id}
                            </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-8 mb-8">
                            ...
                            <div className="bg-purple-50 p-4 rounded-xl">
                                <p className="text-sm font-bold text-purple-700">Passenger</p>
                                <p className="text-2xl font-bold text-purple-800">{activeRide.rider?.name || 'Guest'}</p>
                            </div>
                        </div>
                            ...
                            <div className="bg-yellow-50 p-6 rounded-xl border border-yellow-200">
                                ...
                                <div className="flex gap-4">
                                    <input
                                        ...
                                        maxLength={4}
                                    />
                                    <button onClick={handleStartRide} className="bg-black text-white px-8 rounded-lg font-bold hover:bg-gray-800 transition shadow-lg whitespace-nowrap">
                                        Start Ride
                                    </button>
                                </div>
                            </div>
                                ...
                                availableRides.map(ride => (
                                    <div key={ride.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl hover:border-black/5 transition duration-300 group">
                                        ...
                                        <button
                                            ...
                                            className="w-full bg-white border-2 border-black text-black font-bold py-3 rounded-xl hover:bg-black hover:text-white transition group-hover:bg-black group-hover:text-white"
                                        >
                                            Accept Request
                                        </button>
                                    </div>
